...
export type EligibilityResult = {
  ...
  refundMethods: {
    method: "VOX_CREDIT" | "SHARE_POINTS" | "ORIGINAL_PAYMENT";
    amountCents: number;
    points?: number;
    eta: string;
  }[];
...
export function evaluateCancellation(
  ...
  const fail = (code: EligibilityResult["code"], reason: string): EligibilityResult => ({
    eligible: false,
    code,
    reasons: [reason, ...reasons],
    minutesToShowtime: minutes,
    refundableTicketIds: [],
    amounts: empty,
    refundMethods: [],
    swapAllowed: false,
  ...
  if (b.status === "cancelled" || b.status === "refunded" || b.status === "swapped")
    return fail("BOOKING_ALREADY_CANCELLED", `Booking ${b.bookingId} has already been ${b.status}.`);
  if (minutes < 0)
    return fail("SHOW_STARTED", "The movie has already started, so this booking can no longer be cancelled.");
  if (b.ticketsCollected || b.status === "collected" || b.tickets.some((t) => t.Status === "used"))
    return fail(
      "TICKETS_USED",
      "Tickets have already been collected at the kiosk or scanned, so they cannot be refunded.",
    );
  if (minutes < policy.cutoffMinutes)
    return fail(
      "CUTOFF_PASSED",
      `Cancellations must be requested at least ${policy.cutoffMinutes} minutes before the movie starts; this one starts in ${minutes} minute${minutes === 1 ? "" : "s"}.`,
    );
  if (b.isThirdParty)
    return fail(
      "BOOKING_NOT_ELIGIBLE",
      "This booking was made through a third-party partner; please contact the partner to cancel.",
    );
  const bankOffer =
    b.appliedOffers.some((o) => o.type === "bank" || o.type === "partner") ||
    b.tickets.some((t) => t.DealDefinitionId?.startsWith("BANK"));
  if (bankOffer)
    return fail(
      "BOOKING_NOT_ELIGIBLE",
  ...
  if (b.concessions.some((c) => c.activated))
    return fail(
      "BOOKING_NOT_ELIGIBLE",
      "Food & drinks on this booking have already been activated (Prepare Now / QR), so the booking cannot be cancelled.",
    );
  ...
  if (requestedTicketIds?.length) {
    if (!policy.allowPartial)
      reasons.push("Partial cancellation is not allowed; the whole booking will be cancelled.");
  ...
  const concessionsCents =
    full && policy.refundConcessions ? b.concessions.reduce((a, c) => a + c.FinalPriceCents, 0) : 0;
...
export function evaluateSwap(
  ...
  target: {
    ...
    ticketPriceCentsByCode: Record<string, number>;
  },
  policy: PolicyConfig = DEFAULT_POLICY,
) {
  ...
  if (targetMinutes < policy.cutoffMinutes)
    return {
      allowed: false,
      reasons: ["The new session starts too soon to complete a swap."],
      differenceCents: 0,
      code: "CUTOFF_PASSED" as const,
    };
  const count = b.tickets.filter((t) => t.Status === "valid").length;
  if (target.seatsAvailable < count)
    return {
      allowed: false,
      reasons: [`The new session only has ${target.seatsAvailable} seats left; you need ${count}.`],
      differenceCents: 0,
      code: "BOOKING_NOT_ELIGIBLE" as const,
    };
  ...
  return {
    ...
    differenceCents,
    code: "OK" as const,
    refundCents: base.amounts.ticketsCents,
    newTicketsCents: newTotal,
  };
}
