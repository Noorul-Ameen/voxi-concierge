...
The [dated acceptance record](../../../docs/14-enhancement-acceptance.md) tracks the current run. V9 has **49/51 provider passes and 0 partial deterministic-audit failures**, but one wrong spoken price remains a real failure. V10's **17 variants × 3 repeats have been invoked** on validation version `agtvrsn_2201m243pa2qfq8b35nsrae8swha`; the invocation response and results are pending. All 17 tests are attached; IDs are in `validation-test-map-v10.json`. Local source validation passed **224 tests across 34 files with no failures/skips, 11 builds, and lint with 0 errors and 212 warnings**. The next PR CI result is pending; its historical first run had 188 passes and 1 skip. Main is not promoted and the application is not deployed. Actual voice listening is unverified; seven Railway variables were staged with deployment skipped, and private passwords were not committed.
