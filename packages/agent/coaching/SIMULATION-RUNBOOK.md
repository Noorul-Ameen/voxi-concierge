...
The provider enum is `mocking_strategy: "all"` and `fallback_strategy: "raise_error"`. All 56 attached server/client tools receive test-specific mocks; conditional mocks have a final error response so an unmatched parameter cannot use a shared provider mock. Do not replace either setting with `selected` or `call_real_tool` to make a failing test pass. System/workflow tools are not mockable. The only permitted native built-in is `skip_turn`, which waits silently and cannot call the backend or change an order; all transfer, end-call and workflow actions remain disabled for this suite. A skip-turn trace does not establish a successful booking, food update, seat map or other backend result. Verify this exact built-in allowlist before each run.

The [official skip-turn documentation](https://elevenlabs.io/docs/eleven-agents/customization/tools/system-tools/skip-turn) defines it under `prompt.built_in_tools.skip_turn` with `type: "system"`, `name: "skip_turn"` and `params.system_tool_type: "skip_turn"`. Check actual English and Arabic voice separately: after an explicit request for a moment, one short acknowledgement is followed by silence instead of a no-input follow-up. The existing eight-second turn timeout, 180-second inactivity limit and authoritative hold expiry remain unchanged.
