...
export function actionContext(type: string, result: { ok: boolean; speech?: string; data?: Record<string, unknown>; action?: { status: string; type: string; result?: Record<string, unknown> }; error?: string }): string {
  ...
  const safe = Object.fromEntries(Object.entries(data).filter(([key]) => ["bookingState", "summary", "proposal", "proposalRef", "admission", "confirmationId", "seats", "totalCents", "expiresAtUtc", "needs", "combinedCheckout", "bookingReference", "linked", "reason"].includes(key)));
