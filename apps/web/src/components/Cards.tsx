...
export function refundChoiceCommand(meta: Record<string, any>, method: unknown): Record<string, unknown> | undefined {
  ...
  if (meta.journey === "swap") {
    if (typeof meta.targetSessionKey !== "string" || !meta.targetSessionKey || typeof meta.refundChoiceProof !== "string" || !meta.refundChoiceProof || typeof meta.keepSeatsIfPossible !== "boolean") return;
    return { type: "swap.refund.choose", bookingId: meta.bookingId, targetSessionKey: meta.targetSessionKey, refundMethod: method, keepSeatsIfPossible: meta.keepSeatsIfPossible, refundChoiceProof: meta.refundChoiceProof };
  }
...
function SwapRefundReview({ summary: s, lang }: { summary: any; lang: Lang }) {
  const ar = lang === "ar";
  const seats = Array.isArray(s.selectedSeats) ? s.selectedSeats.map((x: any) => `${x.Row}${x.Number}`).join(", ") : "";
