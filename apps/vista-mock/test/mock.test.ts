...
describe("auth (Apigee)", () => {
  ...
  it("rejects expired token with the documented fault", async () => {
    ...
    const res = await t.app.request("/vistatickets/vista/v2/OData/Cinemas", { headers: { "x-api-key": t.cfg.auth.apiKey, authorization: `Bearer ${tok}` } });
    expect(res.status).toBe(401);
...
describe("OData reference data", () => {
  ...
  it("filters films with the documented now-showing/coming-soon expression", async () => {
    const r = await t.get("/OData/Films?$format=json&$filter=IsScheduledAtCinema eq true or OpeningDate gt DATETIME '2026-09-01T00:00:00'");
  ...
  it("returns sessions for a cinema with $expand=Attributes", async () => {
    const r = await t.get("/OData/Sessions?$format=json&$filter=CinemaId eq '0002'&$expand=Attributes&$top=5");
...
describe("order lifecycle with seat holds", () => {
  ...
  it("finds an upcoming session and ticket types", async () => {
    ...
    ticketCode = tt.json.Tickets.find((x: any) => /ADULT/.test(x.Description) && x.AreaCategoryCode === "0000000002").TicketTypeCode;
  });
  it("adds tickets with auto-allocation, then re-selects seats, adds F&B, pays and returns a booking", async () => {
    ...
    const add = await t.post("/Ticketing/Order/tickets", { UserSessionId: usid, CinemaId: cinemaId, SessionId: sessionId, TicketTypes: [{ TicketTypeCode: ticketCode, Qty: 2 }], ReturnOrder: true });
    ...
    const plan = await t.get(`/Data/Cinemas/${cinemaId}/sessions/${sessionId}/seat-plan?userSessionId=${usid}`);
    const all = plan.json.SeatLayoutData.Areas.flatMap((a: any) => a.Rows.flatMap((r: any) => r.Seats.map((s: any) => ({ ...s, row: r.PhysicalName }))));
    ...
    const seats = await t.post("/Ticketing/Order/seats", { UserSessionId: usid, CinemaId: cinemaId, SessionId: sessionId, SelectedSeats: free.map((s: any) => ({ Row: s.row, Number: s.Id })) });
    expect(seats.json.Result).toBe(0);
    expect(seats.json.Order.Sessions[0].Tickets.map((x: any) => x.SeatData).sort()).toEqual(free.map((s: any) => `${s.row}${s.Id}`).sort());
    const fnb = await t.post("/Ticketing/Order/concessions", { UserSessionId: usid, CinemaId: cinemaId, Concessions: [{ ItemId: "160", Quantity: 1, Modifiers: ["COKEZ"] }] });
    ...
    const declined = await t.post("/Ticketing/order/payment", { UserSessionId: usid, CustomerEmail: "qa@example.com", CustomerName: "QA Tester", CustomerPhone: "0500000000", PaymentInfoCollection: [{ PaymentValueCents: total, PaymentTenderCategory: "CREDIT", CardNumber: "4000000000000002", CardExpiryMonth: "12", CardExpiryYear: "30" }] });
    expect(declined.status).toBe(200);
    expect(declined.json.Result).toBe(1);
    expect(declined.json.ErrorDescription).toMatch(/declined/);
    const pay = await t.post("/Ticketing/order/payment", { UserSessionId: usid, CustomerEmail: "qa@example.com", CustomerName: "QA Tester", CustomerPhone: "0500000000", PaymentInfoCollection: [{ PaymentValueCents: total, PaymentTenderCategory: "CREDIT", CardNumber: "4111111111111111", CardExpiryMonth: "12", CardExpiryYear: "30" }] });
    expect(pay.json.Result).toBe(0);
    expect(pay.json.VistaBookingId).toHaveLength(7);
    // idempotent re-pay returns the same booking
    const again = await t.post("/Ticketing/order/payment", { UserSessionId: usid, CustomerEmail: "qa@example.com", CustomerName: "QA Tester", CustomerPhone: "0500000000", PaymentInfoCollection: [{ PaymentValueCents: total, PaymentTenderCategory: "CREDIT", CardNumber: "4111111111111111" }] });
    ...
    const sold = plan2.json.SeatLayoutData.Areas.flatMap((a: any) => a.Rows.flatMap((r: any) => r.Seats.filter((s: any) => free.some((f: any) => f.row === r.PhysicalName && f.Id === s.Id))));
  ...
  it("prevents two orders from holding the same seat concurrently", async () => {
    ...
    const mk = async (usid: string) => {
      await t.post("/Ticketing/Order/tickets", { UserSessionId: usid, CinemaId: cinemaId, SessionId: sessionId, TicketTypes: [{ TicketTypeCode: ticketCode, Qty: 1 }], UserSelectedSeatingSupported: true });
      return t.post("/Ticketing/Order/seats", { UserSessionId: usid, CinemaId: cinemaId, SessionId: sessionId, SelectedSeats: [{ Row: free.row, Number: free.Id }] });
    };
    const results = await Promise.all(Array.from({ length: 6 }, (_, i) => mk(`race-${Date.now()}-${i}`)));
    const ok = results.filter((r) => r.json.Result === 0);
    expect(ok).toHaveLength(1);
    expect(results.filter((r) => r.json.Result !== 0).every((r) => /no longer available|concurrently/.test(r.json.ErrorDescription))).toBe(true);
  });
  it("cancel order releases seats", async () => {
    const usid = `cancel-${Date.now()}`;
    const add = await t.post("/Ticketing/Order/tickets", { UserSessionId: usid, CinemaId: cinemaId, SessionId: sessionId, TicketTypes: [{ TicketTypeCode: ticketCode, Qty: 1 }] });
    ...
    const plan = await t.get(`/Data/Cinemas/${cinemaId}/sessions/${sessionId}/seat-plan`);
    const s = plan.json.SeatLayoutData.Areas.flatMap((a: any) => a.Rows.filter((r: any) => r.PhysicalName === seat.SeatRowId).flatMap((r: any) => r.Seats)).find((x: any) => x.Id === seat.SeatNumber);
...
describe("offers engine", () => {
  it("applies a promo code and rejects combining with a bank offer", async () => {
    ...
    const code = tt.json.Tickets.find((x: any) => /ADULT/.test(x.Description) && x.AreaCategoryCode === "0000000002").TicketTypeCode;
    const add = await t.post("/Ticketing/Order/tickets", { UserSessionId: usid, CinemaId: "0005", SessionId: monday.SessionId, TicketTypes: [{ TicketTypeCode: code, Qty: 2 }] });
...
describe("bookings & refunds", () => {
  it("searches by booking id, phone and email", async () => {
    ...
    const byEmail = await t.post("/RESTBooking.svc/booking/search", { Email: "James.Whitfield@example.com", UpcomingOnly: true });
    expect(byEmail.json.Bookings.every((b: any) => b.Customer.Email === "james.whitfield@example.com")).toBe(true);
  });
  it("refunds to VOX credit idempotently and rejects a second refund", async () => {
    ...
    const bal0 = (await t.get("/RESTLoyalty.svc/member/SHR400551/balances")).json.Balances.find((x: any) => x.BalanceTypeId === "VOX_REWARDS").ValueCents;
    const ref = `TEST-${Date.now()}`;
    const r1 = await t.post("/RESTBooking.svc/booking/refund", { BookingId: "OKGRP33", RefundTenderCategory: "EWALLET", Reference: ref, TicketIds: ["1"] });
    expect(r1.json.Result).toBe(0);
    expect(r1.json.Booking.Status).toBe("partially_refunded");
    const r2 = await t.post("/RESTBooking.svc/booking/refund", { BookingId: "OKGRP33", RefundTenderCategory: "EWALLET", Reference: ref, TicketIds: ["1"] });
    expect(r2.json.Idempotent).toBe(true);
    expect(r2.json.Refund.Reference).toBe(ref);
    const bal1 = (await t.get("/RESTLoyalty.svc/member/SHR400551/balances")).json.Balances.find((x: any) => x.BalanceTypeId === "VOX_REWARDS").ValueCents;
    expect(bal1 - bal0).toBe(r1.json.Refund.AmountCents);
    const full = await t.post("/RESTBooking.svc/booking/cancel", { BookingId: "OKGRP33", RefundTenderCategory: "EWALLET" });
    expect(full.json.Booking.Status).toBe("refunded");
    const again = await t.post("/RESTBooking.svc/booking/cancel", { BookingId: "OKGRP33", RefundTenderCategory: "EWALLET" });
