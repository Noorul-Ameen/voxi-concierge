...
const v1ok = (body: Record<string, unknown>) => ({
  ExtendedResultCode: 0,
  ...body,
  Result: 0,
  ErrorDescription: null,
});
const v1err = (e: VistaError) => ({
  ExtendedResultCode: e.extended,
  Result: e.result,
...
export function createApp(db: Db, cfg: MockConfig) {
  ...
  const odata = (rows: Record<string, unknown>[], c: Context, expandable?: string[]) => {
    ...
    try {
      ...
      return c.json(
        { error: { code: "BadRequest", message: { lang: "en-US", value: (e as Error).message } } },
        400,
      );
    }
  };
  ...
  api.get("/OData/ScheduledFilms", async (c) => {
    ...
    return odata(
      rows.map(({ f, sf }) => ({
        ...filmJson(f, sf.cinemaId),
        FirstShowtime: sf.firstShowtime?.toISOString().slice(0, 19),
        LastShowtime: sf.lastShowtime?.toISOString().slice(0, 19),
      })),
      c,
      ["Cast"],
    );
  ...
  api.get("/OData/Sessions", async (c) => {
    ...
    const films = await db
      .select()
      .from(S.films)
      .where(rows.length ? inArray(S.films.hoCode, [...new Set(rows.map((r) => r.hoCode))]) : sql`false`);
    return odata(
      rows.map((r) =>
        sessionJson(
          r,
          attrs,
          films.find((f) => f.hoCode === r.hoCode),
        ),
      ),
      c,
      ["Attributes"],
  ...
  api.get("/Data/Cinemas/:cinemaId/sessions/:sessionId/tickets", async (c) => {
    ...
    if (!sess)
      return c.json({
        ResponseCode: 1,
        ErrorDescription: `Session ${sessionId} not found for Cinema ${cinemaId}`,
        Tickets: [],
      });
    const rows = await db
      .select()
      .from(S.ticketTypes)
      .where(and(eq(S.ticketTypes.cinemaId, cinemaId), eq(S.ticketTypes.experience, sess.experience)));
  ...
  api.get("/Data/Cinemas/:cinemaId/sessions/:sessionId/seat-plan", async (c) => {
    ...
    try {
      ...
      if (e instanceof VistaError)
        return c.json({ SeatLayoutData: null, ResponseCode: e.extended, ErrorDescription: e.message });
      throw e;
    }
  });
  ...
  api.get("/Data/concession-items-grouped-by-tabs", async (c) => {
    ...
    const items = rows
      .filter((r) => !r.cinemaId || r.cinemaId === cinemaId)
      .sort((a, b) => (a.displaySequence ?? 0) - (b.displaySequence ?? 0));
    const tabs = [...new Set(items.map((i) => i.tab))].map((tab) => ({
      Name: tab,
      NameAlt: tab,
      Items: items.filter((i) => i.tab === tab).map(concessionJson),
    }));
  ...
  const orderResponse = async (userSessionId: string) => {
    ...
    const session = order.sessionId
      ...
      : null;
    const film = session
      ? ((await db.select().from(S.films).where(eq(S.films.hoCode, session.hoCode)))[0] ?? null)
      : null;
    return v1ok({
      Order: orderJson(order, session, film),
      SessionStatuses: session
        ? [
            {
              SessionId: session.sessionId,
              NumberOfSeatsAvailable: session.seatsAvailable,
  ...
  api.post("/Ticketing/Order/tickets", async (c) => {
    ...
    const r = await addTickets(
      db,
      { ...body, UserSessionId: String(body.UserSessionId), SessionId: String(body.SessionId) },
      cfg.order,
    );
    const film = (await db.select().from(S.films).where(eq(S.films.hoCode, r.session.hoCode)))[0] ?? null;
    return c.json(
      v1ok({
        Order: body.ReturnOrder === false ? undefined : orderJson(r.order, r.session, film),
        SeatData: "",
        AreaSummaryData: "",
        SeatDataLength: 0,
        SeatsNotAllocated: !r.order.seatsAllocated,
        ...
        SessionStatuses: [
          {
            ...
            NumberOfSeatsAvailable: r.availableSeats,
            MaximumNumberOfSeatsPerRow: 0,
          },
        ],
  ...
  api.post("/Ticketing/Order/seats", async (c) => {
    const body = await c.req.json();
    const selected = Array.isArray(body.SelectedSeats)
      ? body.SelectedSeats
      : body.SelectedSeats
        ? [body.SelectedSeats]
        : [];
    const r = await setSeats(
      db,
      {
        UserSessionId: String(body.UserSessionId),
  ...
  api.post("/Ticketing/Order/concessions", async (c) => {
    ...
    return c.json({
      ...(await orderResponse(r.order.userSessionId)),
      FailedConcessions: r.failed.length ? r.failed : null,
    });
  });
  ...
  api.get("/Ticketing/order/:userSessionId", async (c) =>
    c.json(await orderResponse(c.req.param("userSessionId"))),
  );
    ...
    return c.json(
      v1ok({
        ...
        PaymentInfoCollection: b.payments.map((p) => ({
          ...
          PaymentStatus: "",
          BillingValueCents: p.PaymentValueCents,
    ...
    return c.json(
      v1ok({
        Refund: {
          ...
          TenderCategory: r.refund.tenderCategory,
          TicketIds: r.refund.ticketIds,
          Status: r.refund.status,
          CreatedUtc: r.refund.createdAt.toISOString(),
        },
  ...
  api.post("/RESTBooking.svc/booking/cancel", async (c) => {
    ...
    const body = await c.req.json();
    const r = await refundBooking(db, {
      BookingId: body.BookingId,
      RefundTenderCategory: body.RefundTenderCategory ?? "EWALLET",
  ...
  api.post("/RESTBooking.svc/booking/link", async (c) => {
    ...
    await db
      .update(S.bookings)
      .set({ swappedToBookingId: body.ToBookingId, status: "swapped", updatedAt: new Date() })
      .where(eq(S.bookings.vistaBookingId, body.FromBookingId));
    await db
      .update(S.bookings)
      .set({ swappedFromBookingId: body.FromBookingId, updatedAt: new Date() })
      .where(eq(S.bookings.vistaBookingId, body.ToBookingId));
    return c.json(v1ok({}));
  });
  ...
  api.get("/RESTLoyalty.svc/member/:memberId/balances", async (c) => {
    const acct = (
      await db
        ...
        .where(eq(S.loyaltyAccounts.memberId, c.req.param("memberId")))
    )[0];
      ...
      v1ok({
        ...
        Balances: [
          ...
          },
          {
            ...
            Name: "VOX credit",
            Points: null,
            ValueCents: acct.voxRewardsBalanceCents,
          },
        ],
      }),
    );
  ...
  api.get("/RESTLoyalty.svc/member/:memberId/ledger", async (c) => {
    const rows = await db
      ...
      .where(eq(S.loyaltyLedger.memberId, c.req.param("memberId")))
      .orderBy(desc(S.loyaltyLedger.createdAt))
      .limit(20);
    return c.json(
      v1ok({
        Entries: rows.map((r) => ({
  ...
  api.get("/offers/v1/offers", async (c) => {
    ...
    const out = rows
      ...
      .map((o) => {
        const e = evaluateOffer(o as unknown as Offer, {
          ...
          experience: q.experience ?? session?.experience,
          hoCode: session?.hoCode,
          showtime: session ? session.showtime.toISOString().slice(0, 19) : undefined,
          memberId: q.memberId,
          tier: acct?.tier,
    ...
    return c.json({
      offers: out.map((o) => ({
        ...
        shortDescriptionAlt: o.shortDescriptionAlt,
        terms: o.terms,
        type: o.type,
        imageUrl: o.imageUrl,
        rules: o.rules,
        ...
        perMemberLimit: o.perMemberLimit,
  ...
  api.post("/offers/v1/offers/:id/eligibility", async (c) => {
    const o = (
      await db
        .select()
        .from(S.offers)
        .where(eq(S.offers.id, c.req.param("id")))
    )[0];
    ...
    const acct = body.memberId
      ...
      : undefined;
    const e = evaluateOffer(o as unknown as Offer, {
      cinemaId: session?.cinemaId ?? body.cinemaId,
      experience: session?.experience ?? body.experience,
      hoCode: session?.hoCode,
      showtime: session ? session.showtime.toISOString().slice(0, 19) : body.showtime,
