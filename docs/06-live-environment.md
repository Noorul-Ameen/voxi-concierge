...
## Showtime refresh — 13 Sep 2026
Fresh capture from uae.voxcinemas.com via `apps/scraper/browser-capture.js` in the user's Chrome (exported as a JSON download, merged with `merge-capture.cjs`, normalised with `pnpm scrape --from-snapshot`): 23 cinemas (new: City Centre Me'aisem `0110`), 93 films, 4,401 sessions for 13–23 Sep (late-night prints roll to the next business day; ~250 rows for films without an HO code, e.g. the Avengers Endgame re-release, are dropped by the normaliser). Fixed in `normalize.ts`: cinema pages are matched by exact slug (the old prefix match gave every "City Centre …" cinema Ajman's address) and the curated address/coordinates/parking text now lives in `CINEMA_OVERRIDES` so a re-capture cannot regress it. KB cinema docs rebuilt (`pnpm --filter @voxi/agent kb:build`; the generator's Arabic glossary now uses the Virtual Assistant name). Production reseeded with `SEED_FORCE=true` for one deploy (demo bookings and balances reset), then the flag set back to `false`. The seeder's week-shift keeps this capture valid until 20 Sep; after that either recapture or reseed to shift forward.
